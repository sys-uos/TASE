...
class BirdEstimatorDirected():
    ...
    def __init__(self, CSV_NODE_LOCATIONS="./data/preprocessed/node_locations.csv"):
        pass
        # self.location_data_list: [Recording_Node] = Location.parse_node_locations(CSV_NODE_LOCATIONS)
        # self.location_data_list = Location.convert_wgs84_to_utm(self.location_data_list, zone_number=33, zone_letter='N')
        # print("converted locations to UTM33N")
        # self.df = self.parse_classification(CSV_CLASSIFICATIONS)
        # self.fill_missing_timeeintervals_with_zero(interval=interval)  # todo make var for saved pkl file
        ...
        # }
        #
        # self.load_pkl_data(dir="/media/caspar/external/Zwergohreulen/2024/pkl/")
        # print("loaded data")
    ...
    def add_nodes_with_coordinates(self, device_list: [Recording_Node]):
        """
    ...
    def set_weight_to_timestamp(self, epoch_ts):
        ...
        for dev in self.location_data_list:
            df = self.data[dev.id]
            # print(df.iloc[0]['start'])
            # from TASE.plots.paper_plots import deployment_duration
            # deployment_start, deployment_end = deployment_duration()
            # print(df.iloc[0]['start']- deployment_start)
            # print(dev.id, epoch_ts)
            # print(dev.id, df.loc[df['start'] == epoch_ts, 'confidence'])
    ...
    def delauny(self, e_delta: float = 0.05, nodes_above_threshold: float = 0.925, nodes_below_threshold: float = 0.075):
    ...
    def remove_long_edges(self, threshold_meter : float = 500.0):
        # 1.3 Set weight of the nodes to the difference between weight
    ...
    def check_criteria(self, G : nx.DiGraph, path, leaf_threshold:float, weight_threshold:float, breeding_birds_distance):
    ...
    def tase(self, threshold_R: float = 0.5, threshold_B: float = 0.1, TS_delta: float = 0.1, threshold_T=np.inf):
        ...
        def bfs_with_all_predecessors(G: nx.Graph, root, leaf_threshold: float, weight_threshold: float,
            ...
            while queue:
                ...
                # print("Node:", current_node, "Predecessors:", predecessors[current_node], len(G[current_node]))  # Print or process the current node and its predecessors
                ...
                # neighbors = dict(sorted(neighbors.items(), key=lambda x: x[1]['weight'], reverse=True)) # might be optional

                # print(current_node)
                # if current_node == 25:
                #     print(list(neighbors), visited)
                for neighbor in neighbors:
                    if neighbor not in visited and \
                            self.check_criteria(G, predecessors[current_node] + [current_node, neighbor],
                                                leaf_threshold,
