...
from TASE.deployment.species import evaluation_specs, Phoenicurs_phoenicurus, Turdus_philomelos, Troglodytes_troglodytes
from TASE.deployment.utils import deployment_node_locations, deployment_duration
...
def plot_evaluation():
    ...
    for spec in evaluation_specs()[-2:]:
        ...
        for params in get_TASE_ParameterSet(spec):
            ...
            odir = f"./plots/final/evaluation/{fn_spec}/"
...
def plot_evaluation_impact_of_time():
    ...
    for spec in [Phoenicurs_phoenicurus(), Turdus_philomelos()]:
        # --- Define output directory --- #
        fn_spec = spec.lat_name.replace(" ", "_")
        odir = f"./plots/final/evaluation/interference/{fn_spec}/"
        os.makedirs(odir, exist_ok=True)
...
def plot_methodological_errors(spec=Phoenicurs_phoenicurus(), rinterval=0, font_size=20):
    ...
    for time_interval in [5, 41, 2268, 2562, 2621]:
        # --- Define output directory --- #
        odir = f"./plots/final/evaluation/methodological/"
        os.makedirs(odir, exist_ok=True)
        fn_spec = spec.lat_name.replace(" ", "_")
...
def plot_evaluation_spec_over_time():
    # --- Define deployment duration --- #
    deployment_start, deployment_end = deployment_duration()
    ...
    # --- Define output directory --- #
    odir = f"./plots/final/evaluation/{fn_spec}_short_timespans/"
    os.makedirs(odir, exist_ok=True)
